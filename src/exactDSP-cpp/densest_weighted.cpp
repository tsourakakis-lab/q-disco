...
//    SO WE CAN'T PUT FRACTIONALS, INSTEAD, JUST MULTIPLY EVERYTHING BY 100 OR 1000...
    ...
    int opt_density_estimate_upper = atoi(argv[4]);
    char* edgelist=argv[5];
    ...
    // cout<< "phase 0";
    double m_node_density=0, m_density=0;
    ...
    while (lambda_l<lambda_r) {
        ...
        double edge_density = (double)l/ACCURACY/lambda_acc - float(res_node_weight)/subg.size()*lambda/lambda_acc;
        cout << "size:" << subg.size() << ", Density is " << (double)l/ACCURACY/lambda_acc << ", node weight density: " << float(res_node_weight)/subg.size() << ", edge density:" << edge_density << endl;
        if (float(res_node_weight)/subg.size()>theta && m_density < edge_density){
            m_density = edge_density;
            m_node_density = float(res_node_weight)/subg.size();
            optimal_S = subg;
        }
    ...
    cout << "final optimal density:" << m_density << " with node density:" << m_node_density << endl;
